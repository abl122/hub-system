...
const proceedToRecoveryMethod = () => {
  if (!recoveryCnpj.value) {
    recoveryError.value = 'Por favor, informe seu CNPJ'
    return
  }
  recoveryError.value = ''
  fetchContacts()
...
const fetchContacts = async () => {
  ...
  recoverySuccess.value = ''
  try {
    const { data } = await passwordRecoveryService.getContacts(recoveryCnpj.value)
...
const verifyCodeAndReset = async () => {
  ...
  try {
    ...
    setTimeout(() => {
      closeRecoveryModal()
      // Preencher o CNPJ no login
      cnpj.value = recoveryCnpj.value
    }, 2000)
  } catch (err: any) {
...
/**
 * Formatar CNPJ com máscara 00.000.000/0000-00
 */
...
const handleLogin = async () => {
  ...
  try {
    const success = await authStore.loginPortal(cnpj.value, password.value)
    if (success) {
...
<template>
  <div class="login-container">
    <div class="login-card">
      <h2>MK-Edge Portal</h2>
      <p class="subtitle">Acesso para provedores</p>
      ...
      <div class="modal-card" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div v-if="recoveryStep === 'identification'" class="recovery-step">
            <p class="step-description">Informe seu CNPJ para recuperar a senha</p>
            
            <div class="form-group">
              <label for="recovery-cnpj">CNPJ</label>
              <input
                id="recovery-cnpj"
